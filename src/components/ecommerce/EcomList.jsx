...
function EcomList() {
  ...
  function addToCart(prodId) {
    ...
    //fetch the product from the allProducts array using the productId
    let fetchedProduct = allProducts.find(
      (eachProduct) => eachProduct.prodId == prodId
    );
    //add the product to the cart array  which is a state variable
  ...
  function deleteCartItem(cartId) {
    ...
    setCart(filteredCart); //we cannot do this--->cart = filteredCart
  }
            ...
            <div>
              {cart.length == 0 ? (
                ...
                <>
                  ...
                  <p>
                    Total Cost:
                    {cart.reduce((sum, eachItem) => sum + eachItem.prodCost, 0)}
                  </p>
                </>
              )}
            </div>
